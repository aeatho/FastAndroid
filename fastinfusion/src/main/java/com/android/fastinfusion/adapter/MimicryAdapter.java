...
public class MimicryAdapter extends BaseAdapter {
  ...
  public MimicryAdapter(Context context, int columnCount, ListAdapter wrappedAdapter) {
  ...
  public void setOnMimicryItemClicked(OnMimicryItemClickedListener onGridMimicItemClicked) {
  ...
  public int getCount() {
    ...
    // columnCount columns
    return (int) Math.ceil(wrappedAdapter.getCount() / (double) columnCount);
  }
  ...
  public View getView(int position, View convertView, ViewGroup parent) {
    ...
    for (int i = 0; i < columnCount; i++) {
      if (position * columnCount + i < wrappedAdapter.getCount()) {
        View recycleView = gridMimicRow.getChildCount() >= i ? gridMimicRow.getChildAt(i) : null;
        views[i] = wrappedAdapter.getView(position * columnCount + i, recycleView, parent);
      } else {
        views[i] = null;
      }
    }
        ...
        if (view == null) {
          ...
          view.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
              if (itemClicked != null) itemClicked.onItemClicked(item);
            }
          });
        }
        addView(view, new LayoutParams(0, LayoutParams.WRAP_CONTENT, 1F));
      }
    }
  }
